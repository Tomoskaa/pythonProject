...
if __name__ == "__main__":
    ...
    col_index = int(input())
    broj_na_drva = int(input())
    kriterium = input()
    entry = [int(element) for element in input().split(' ')]
    entry = [entry[i] for i in range(0, len(entry)) if i != col_index]

    # dataset = [[dataset[i][j] for j in range(0, len(dataset[i]))] for i in range(0, len(dataset))]
    ...
    test_y = [test_set[i][-1] for i in range(0, len(test_set))]

    clf = RandomForestClassifier(n_estimators=broj_na_drva, criterion=kriterium, random_state=0)
    # clf.fit(train_x, train_y)

    train_x_2 = list()
    for t in train_x:
        sample = [t[i] for i in range(len(t)) if i != col_index]
    ...
    print(clf.predict_proba([entry])[0])
